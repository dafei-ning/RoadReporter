        ...
        database.child("events").child(key).setValue(event, new DatabaseReference.CompletionListener() {
            ...
            public void onComplete(DatabaseError databaseError, DatabaseReference databaseReference) {
                if (databaseError != null) {
                    ...
                    Toast toast = Toast.makeText(getContext(), "该位置已经被报告（添加数据库）", Toast.LENGTH_SHORT);
                    toast.show();
...
//        });
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        marker.icon(BitmapDescriptorFactory.fromResource(R.drawable.witcher3_icon3));
    ...
    private void animateDialog(View dialogView, boolean open, final Dialog dialog) {
        ...
        int endRadius = (int) Math.hypot(w, h);   // 获得屏幕的斜线

        int cx = (int) (fab_report.getX() + (fab_report.getWidth()/2));
        int cy = (int) (fab_report.getY())+ fab_report.getHeight();
    ...
    private void setupRecyclerView(View dialogView) {
        ...
        listItems.add(new Item(Config.POLICE, R.drawable.witcher3_icon));
        listItems.add(new Item(Config.TRAFFIC, R.drawable.witcher3_icon2));
